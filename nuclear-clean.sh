...
# safe_remove removes the directory at the given path if it exists and prints a removal message.
safe_remove() {
    if [ -d "$1" ]; then
        echo "🗑️  Removing: $1"
        rm -rf "$1"
...
# safe_remove_file removes the specified file if it exists and prints a status message.
# If the given path is not a regular file (or does not exist) the function does nothing.
safe_remove_file() {
    if [ -f "$1" ]; then
