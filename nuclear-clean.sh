...
# safe_remove removes the specified directory and its contents if it exists, printing a removal message.
safe_remove() {
    if [ -d "$1" ]; then
        echo "🗑️  Removing: $1"
        rm -rf "$1"
...
# safe_remove_file removes the specified file if it exists and echoes a removal message.
safe_remove_file() {
    if [ -f "$1" ]; then
