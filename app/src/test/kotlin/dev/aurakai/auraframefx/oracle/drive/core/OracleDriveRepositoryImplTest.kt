...
class OracleDriveRepositoryImplTest {
    ...
    fun listFiles_returns_mapped_files_on_success() = runBlocking {
        ...
        coEvery { api.listFiles(bucketName = any(), prefix = any()) } returns (retrofitResponse as Response<Nothing>)
    ...
    fun listFiles_returns_empty_on_unsuccessful() = runBlocking {
        ...
        coEvery { api.listFiles(any(), any()) } returns (Response.error(500, errorBody) as Response<Nothing>)
    ...
    fun uploadFile_true_on_success() = runBlocking {
        val tmp = Files.createTempFile("upload", ".bin").toFile().apply { writeBytes(byteArrayOf(1,2,3)) }
        coEvery { api.uploadFile(any(), any(), any()) } returns Response.success(null)
    ...
    fun downloadFile_returns_file_on_success_else_null() = runBlocking {
        ...
        coEvery { api.downloadFile(any(), any()) } returns (Response.success(payload) as Response<Nothing>)
        ...
        coEvery { api.downloadFile(any(), any()) } returns (Response.error(404, err) as Response<Nothing>)
